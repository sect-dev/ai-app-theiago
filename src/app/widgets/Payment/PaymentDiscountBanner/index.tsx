...
const PaymentDiscountBanner:FC<ComponentProps> = ({isMobileVersion}) => {
  ...
  return (
    <div className={clsx("animate-fadeIn relative bg-[#191B2C] banner-bg w-full py-[5px]", {
      "bg-[#8F59FF4F] rounded-[6.40vw] overflow-hidden !py-[3.20vw] px-[4.27vw] !pb-[4.27vw]": isMobileVersion
    })}>
      <span className={clsx("hidden", {
        "!block bg-[#121423] size-[8.53vw] rounded-full absolute left-0 top-1/2 -translate-x-1/2 -translate-y-1/2": isMobileVersion
      })} />
      <span className={clsx("hidden", {
        "!block bg-[#121423] size-[8.53vw] rounded-full absolute right-0 top-1/2 translate-x-1/2 -translate-y-1/2": isMobileVersion
      })} />
