...
const TokenAdvantages = () => {
	...
	return (
							  ...
							  <div className='grid grid-cols-2 gap-[8px] font-bai-jamjuree mb-[8px]' >
								  {ITEMS.map((i) => (
									<div key={i.id} className="p-[16px] rounded-[12px]" style={{ background: i.bg }}>
									  <div className="mb-[12px] bg-[#121423] rounded-[12px] w-fit p-[8px]">
										<Image src={i.image} alt="advantage img" width={i.image.width} height={i.image.height} />
									  </div>
									  <div className="grid grid-row-2 gap-[7px]">
										<span className="font-bold text-[20px] leading-[24px]">{i.title}</span>
										<span className="font-semibold text-[14px] leading-[18px]">{i.desc}</span>
									  </div>
									</div>
								  ))}
							  </div>
