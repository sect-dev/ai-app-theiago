...
const SuccessAuth = () => {
	...
	const characterId = useMemo(() => {
		...
		if (charFromPaywall?.character_id) {
			console.log(
				"Character ID from charFromPaywall:",
			...
			if (pendingActivation) {
				try {
					...
					if (activationData.searchParams) {
						...
						if (charId) {
							console.log(
								"Character ID from pendingSubscriptionActivation:",
								charId
							);
							return charId;
						}
	...
	const getCharacterById = async (id: string) => {
		...
		try {
			...
			console.log(
				"✅ Cleared pendingSubscriptionActivation after successful character loading"
			);
		} catch (error) {
			console.log(error);
		} finally {
		...
		if (characterId) {
			...
			if (typeof window !== "undefined") {
				console.log("Debug localStorage contents:", {
					...
					pendingSubscriptionActivation: localStorage.getItem(
						"pendingSubscriptionActivation"
					),
			...
			<div className="h-hull relative w-full sm:hidden">
				...
				{characterLoading && !charInfo ? (
					...
					<p className="absolute bottom-[20px] left-[20px] block max-w-[70%] font-bai-jamjuree text-[20px] font-semibold leading-[1.2em] tracking-[0.01em] sm:text-[5.33vw]">
						{charInfo?.name} is eager to talk with you!
					</p>
				)}
			</div>
