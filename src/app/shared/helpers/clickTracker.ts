...
export const trackBuyButtonClick = async (): Promise<void> => {
  ...
  try {
    ...
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/click.php?cnv_id=${clickId}&payout=0&cnv_status=buy_button`);
...
export const trackPurchaseSuccess = async (payout: number): Promise<void> => {
  ...
  try {
    ...
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/click.php?cnv_id=${clickId}&payout=${payout}&cnv_status=purchase`);
