...
const FooterComponent: FC<ComponentProps> = ({ paymentPlans, isOrganic }) => {
    ...
    <div className="mx-auto overflow-hidden pb-[10px] pt-[20px] text-center font-bai-jamjuree sm:max-w-[91vw] sm:pb-[2.778vw] sm:pt-[5.56vw]">
      <div className="mb-[15px] flex items-center gap-[10px]">
        <input
          ...
          name="ssd"
          className={clsx(
            "size-[26px] shrink-0 cursor-pointer appearance-none rounded-[8px] border bg-transparent bg-[3px] bg-center bg-no-repeat before:!rounded-[5px]",
            {
              "border-[#049AEF] checked:bg-[url('/images/icons/payment/icon-blue-checked.svg')]":
                isOrganic,
              "border-[#8E59FF] checked:bg-[url('/images/icons/payment/check.svg')]":
                !isOrganic,
            },
          )}
        />
        <p className="text-left text-[12px] font-medium leading-[1.2em] tracking-[-0.04em] text-[#6D6D6D]">
          ...
          <span
            className={clsx(
              ...
              isOrganic ? "bg-blue-text-gradient" : "bg-button-gradient",
            )}
          >
            {" "}
